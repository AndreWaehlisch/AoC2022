...
for jjj, startpos in enumerate([np.array((i, j)) for i, j in zip(*np.where(grid == ord("a")))]):
        ...
        for ipath in pathlist_check:
            ...
            for jpath in icandidates:
                if np.all(jpath.coords == endpos):
                    path_done = jpath
                elif (path_done is None) or (jpath.nsteps < minmin):
    ...
    print(jjj+1, np.sum(grid == ord("a")))
    if path_done:
        print(path_done.nsteps)
        #print(path_done.log)
        if path_done.nsteps < minmin:
                minmin = path_done.nsteps
    else:
        print("nope")
